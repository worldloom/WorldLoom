#!/usr/bin/env python3
"""
Example: Training DreamerV3 World Model

This script demonstrates how to train a DreamerV3 world model using
...
Usage:
    ...
    python examples/train_dreamer.py --data trajectories.npz --resume outputs/checkpoint_best.pt
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="size12m",
        choices=["ci", "size12m", "size25m", "size50m", "size100m", "size200m"],
    ...
    # Determine observation shape and action dim
    # Default for image-based environments
    obs_shape = (3, 64, 64)
    action_dim = 6
    ...
    # Create model
    logger.info(f"Creating DreamerV3 model ({args.size})...")
    model = create_world_model(
        f"dreamerv3:{args.size}",
        obs_shape=obs_shape,
        action_dim=action_dim,
    )
    ...
    # Save final model
    output_path = f"{args.output_dir}/dreamer_final"
